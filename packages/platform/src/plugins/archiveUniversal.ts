...
async function loadWasm(): Promise<void> {
  ...
  try {
    const mod = await import('../../../../crates/fmt-archive-universal/pkg/viewit_fmt_archive_universal.js');
    await mod.default();
...
export async function extractEntry(bytes: Uint8Array, filename: string, entryName: string): Promise<Uint8Array> {
  await loadWasm();
  if (!wasmModule) throw new Error('WASM not loaded');
  return wasmModule.extractEntry(bytes, filename, entryName);
}

export async function extractAll(bytes: Uint8Array, filename: string): Promise<[string, Uint8Array][]> {
  await loadWasm();
  if (!wasmModule) throw new Error('WASM not loaded');
