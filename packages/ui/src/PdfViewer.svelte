<script lang="ts">
  ...
  let native = $derived(!!docProp.native || !!(docProp as { asset_path?: string }).asset_path);
  ...
  function kickOffPdf() {
    if (pdfStarted || !native || !cachePath) return;
    pdfStarted = true;
    pdfjsStatus = 'loading';
    debugLog(`[pdf] kickOffPdf cachePath=${cachePath.slice(0, 80)}`);
