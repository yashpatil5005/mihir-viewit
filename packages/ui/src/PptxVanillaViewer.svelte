<script lang="ts">
  ...
  async function loadDeck() {
    ...
    try {
      ...
      const path = await resolvePptxAssetPath((docProp as any).asset_path, source_uri || undefined);
...
<div class="pvv-root" class:fs={fullscreenState.active}>
  ...
  {#if status === "ready"}
    <p class="muted">
      Rendered by {plugin?.name ?? "PPTX Vanilla"} · view-only (no PowerPoint animations, no editing).
    </p>
  {/if}
</div>
