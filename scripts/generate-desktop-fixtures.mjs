...
// 1. Plain Text
fs.writeFileSync(path.join(FIXTURES_DIR, "sample.txt"), "Hello, ViewIt Offline Desktop!\nLine 2 of test text.\n");

// 2. Markdown
fs.writeFileSync(path.join(FIXTURES_DIR, "sample.md"), "# ViewIt Offline Markdown\n\n- Item 1\n- Item 2\n\n**Bold text** and *italic*.\n");

// 3. JSON
fs.writeFileSync(path.join(FIXTURES_DIR, "sample.json"), JSON.stringify({ app: "ViewIt", status: "offline", count: 42 }, null, 2));

// 4. CSV
fs.writeFileSync(path.join(FIXTURES_DIR, "sample.csv"), "Name,Role,Status\nAlice,Developer,Active\nBob,Designer,Offline\n");

// 5. XML
fs.writeFileSync(path.join(FIXTURES_DIR, "sample.xml"), "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n<root><item id=\"1\">Offline XML</item></root>\n");

// 6. HTML
fs.writeFileSync(path.join(FIXTURES_DIR, "sample.html"), "<!DOCTYPE html><html><body><h1>Desktop HTML Viewer</h1><p>Offline paragraph.</p></body></html>\n");

// 7. VCF
fs.writeFileSync(path.join(FIXTURES_DIR, "sample.vcf"), "BEGIN:VCARD\nVERSION:3.0\nN:Desktop;Tester;;;\nFN:Tester Desktop\nTEL;TYPE=CELL:1234567890\nEND:VCARD\n");

// 8. ICS
fs.writeFileSync(path.join(FIXTURES_DIR, "sample.ics"), "BEGIN:VCALENDAR\nVERSION:2.0\nBEGIN:VEVENT\nSUMMARY:Desktop Release Testing\nDTSTART:20260821T100000Z\nDTEND:20260821T110000Z\nEND:VEVENT\nEND:VCALENDAR\n");

// 9. .desktop entry
fs.writeFileSync(path.join(FIXTURES_DIR, "sample.desktop"), "[Desktop Entry]\nType=Application\nName=ViewIt Desktop\nExec=viewit-desktop\nIcon=viewit\nCategories=Utility;\n");
...
pdfBody += `xref\n0 ${objects.length + 1}\n0000000000 65535 f \n`;
pdfBody += offsets.slice(1).map((o) => `${String(o).padStart(10, "0")} 00000 n \n`).join("");
